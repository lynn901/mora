...
### 2.3 证书管理
- **K8s**：cert-manager + Let's Encrypt（出网仅限 ACME 验证）或私有 CA；自动轮转。
...
## 6. 默认不出网与出网管控（PRD F4.2）

### 6.1 默认不出网
- **网络策略**：K8s NetworkPolicy 限制应用层仅可达数据层（同命名空间/指定 CIDR），默认 deny 所有出网。
- **Docker Compose**：组件仅内部网络通信，不暴露公网端口（仅 API/MCP 经反向代理对外）。
- **模型推理**：TEI/Ollama 本地部署，模型文件预下载挂载，**不在线拉取模型**。

### 6.2 出网白名单（显式授权）
| 出网场景 | 是否允许 | 管控 |
|---|---|---|
| TEI/Ollama 拉模型 | 默认禁止；首次部署离线导入 | 模型文件本地挂载 |
| 外部 Embedding API | 默认禁止 | 须管理员显式配置 `ExternalAPIProvider` + 授权 + 审计 + 可选内容脱敏 |
| ACME 证书（Let's Encrypt） | 可选 | 仅 Ingress 出网，限定 ACME endpoint |
...
| AC-22 数据挂载本地/可备份恢复/不出网 | §3.2/§6/§8 | ✅ |
