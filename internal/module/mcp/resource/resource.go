...
func (r *Registry) List(ctx context.Context) ([]server.ResourceDef, error) {
	...
	defs = append(defs, server.ResourceDef{URI: resourcePrefix + "workspaces", Name: "可见工作区", MimeType: mimeTypeJSON})
	for _, ws := range wss {
		defs = append(defs, server.ResourceDef{
			URI: resourcePrefix + "workspaces/" + ws.ID + "/tree", Name: ws.Name + "-目录树", MimeType: mimeTypeJSON,
		})
		defs = append(defs, server.ResourceDef{
			URI: resourcePrefix + "workspaces/" + ws.ID + "/tags", Name: ws.Name + "-标签", MimeType: mimeTypeJSON,
		})
	}
	return defs, nil
}
